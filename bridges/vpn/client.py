...
def _resolve_target(args) -> bytes | None:
    ...
    RNS.log(
        "[vpn:client] no --target given, waiting for VPN-anchor announce…", RNS.LOG_INFO
    )
...
def run(args) -> None:
    ...
    if target_hash is None:
        sys.exit(
            "no VPN anchor heard within "
            f"{DISCOVERY_TIMEOUT}s — none reachable on the mesh?"
        )
    RNS.log(f"[vpn:client] using anchor {RNS.prettyhexrep(target_hash)}", RNS.LOG_INFO)
    ...
    RNS.log(
        f"[vpn:client] up: {client_ip}/{prefix} via {gateway_ip} " f"(tun={args.tun})",
        RNS.LOG_INFO,
    )
